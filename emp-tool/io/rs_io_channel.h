#ifndef EMP_RS_IO_CHANNEL_H__
#define EMP_RS_IO_CHANNEL_H__
...
class RsSubChannel { public:
	int sock;
	char *buf = nullptr;
	int ptr;
	uint64_t counter = 0;
	RsSubChannel(int sock) : sock(sock) {
		buf = new char[NETWORK_BUFFER_SIZE2];
	}
	~RsSubChannel() { delete[] buf; }
};

class RsSenderSubChannel : public RsSubChannel { public:
	RsSenderSubChannel(int sock) : RsSubChannel(sock) { ptr = 0; }
	...
	void send_data_raw(const void *data, int len) {
		...
		int sent = 0;
		while (sent < len) {
			int res = rwrite(sock, sent + (char *)data, len - sent);
			if (res >= 0)
				sent += res;
			else
...
class RsRecverSubChannel : public RsSubChannel { public:
	RsRecverSubChannel(int sock) : RsSubChannel(sock) { ptr = NETWORK_BUFFER_SIZE2; }
	void flush() {
	...
	void recv_data_raw(void *data, int len) {
		...
		int sent = 0;
		while (sent < len) {
			int res = rread(sock, sent + (char *)data, len - sent);
			if (res >= 0)
				sent += res;
			else
				fprintf(stderr, "error: net_send_data %d\n", res);
...
class RsocketIO: public IOChannel<RsocketIO> { public:
	bool is_server, quiet;
	int send_sock = 0;
	int recv_sock = 0;
	int FSM = 0;
	RsSenderSubChannel *schannel;
	RsRecverSubChannel *rchannel;

	RsocketIO(const char *address, int send_port, int recv_port, bool quiet = true) : quiet(quiet) {
		...
		schannel = new RsSenderSubChannel(send_sock);
		rchannel = new RsRecverSubChannel(recv_sock);
	...
	int server_listen(int port) {
		...
		int reuse = 1;
		rsetsockopt(mysocket, SOL_RDMA, SO_REUSEADDR, (const char *)&reuse, sizeof(reuse));
		...
		int sock = raccept(mysocket, (struct sockaddr *)&dest, &socksize);
		rclose(mysocket);
		return sock;
	}
	int client_connect(const char *address, int port) {
		...
		while (1) {
			...
			if (rconnect(sock, (struct sockaddr *)&dest, sizeof(struct sockaddr)) == 0) break;
			rclose(sock);
			usleep(1000);
		}
		return sock;
	...
	void set_delay_opt(int sock, bool enable_nodelay) {
		if (enable_nodelay) {
			const int one = 1;
			rsetsockopt(sock, IPPROTO_TCP, TCP_NODELAY, &one, sizeof(one));
		} else {
			const int zero = 0;
			rsetsockopt(sock, IPPROTO_TCP, TCP_NODELAY, &zero, sizeof(zero));
		}
	}
